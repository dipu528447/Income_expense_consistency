...
const Difference = (props)=>{
    ...
    <div>
        ...
        <div className='item flex justify-content-between m-6 bg-gray-50 py-2 rounded-r' style={{background:`${(currentTotal-preTotal)*100/currentSalary>0?"red":"green"}`, color:'white'}}>            
            <span className='w-48'>Salary Changed Ratio</span>
            {console.log(currentTotal,preTotal,currentSalary)}
            <span className='w-96'>{(((currentTotal-preTotal)*100)/currentSalary)>0?(((currentTotal-preTotal)*100)/currentSalary).toFixed(2)+'% Decreased':(((preTotal-currentTotal)*100)/currentSalary).toFixed(2)+'% Increased'}</span>
        </div>
    </div>
    );
}
